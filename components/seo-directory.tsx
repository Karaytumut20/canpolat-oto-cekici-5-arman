...
export function SeoDirectory({ eyebrow, title, intro, groups }: { eyebrow: string; title: string; intro: string; groups: Group[] }) {
  return <main className="directory-page">
    <header className="site-header area-header">
      ...
      <nav><Link href="/hizmetler">Hizmetler</Link><Link href="/hizmet-bolgeleri">Bölgeler</Link><Link href="/cekmekoy-oto-cekici">Çekmeköy</Link><Link href="/uskudar-oto-cekici">Üsküdar</Link><Link href="/blog">Bilgi Merkezi</Link></nav>
      <a className="header-call" href={`tel:${siteConfig.phone}`}><Phone size={18} />{siteConfig.phoneDisplay}</a>
      <MobileMenu links={[{ href: "/hizmetler", label: "Hizmetler" }, { href: "/hizmet-bolgeleri", label: "Hizmet bölgeleri" }, { href: "/cekmekoy-oto-cekici", label: "Çekmeköy Oto Çekici" }, { href: "/uskudar-oto-cekici", label: "Üsküdar Oto Çekici" }, { href: "/blog", label: "Bilgi Merkezi" }]} />
    </header>
