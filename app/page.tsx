...
const faqs = [
  ...
  { q: "Hangi bölgelere çekici gönderiyorsunuz?", a: "Çekmeköy merkezli olarak Sancaktepe, Ümraniye ve Sultanbeyli'ye hizmet veriyoruz. Mahalle veya yoldaysanız yön ve en yakın çıkış bilgisini paylaşın." },
...
const priorityDistricts = ["Sancaktepe", "Ümraniye", "Sultanbeyli"] as const;
  ...
  return (
    <main>
      <header className="site-header">
        ...
        <nav aria-label="Ana menü"><Link href="/#hizmetler">Hizmetler</Link><Link href="/hizmet-bolgeleri">Hizmet Bölgeleri</Link><Link href="/cekmekoy-oto-cekici">Çekmeköy</Link><Link href="/sancaktepe-oto-cekici">Sancaktepe</Link><Link href="/blog">Bilgi Merkezi</Link><Link href="/iletisim">İletişim</Link></nav>
        <a className="header-call" href={`tel:${siteConfig.phone}`}><Phone size={18} /> {siteConfig.phoneDisplay}</a>
        <MobileMenu links={[{ href: "/#hizmetler", label: "Hizmetler" }, { href: "/hizmet-bolgeleri", label: "Hizmet Bölgeleri" }, { href: "/blog", label: "Bilgi Merkezi" }, { href: "/iletisim", label: "İletişim & Harita" }]} />
      </header>
      ...
      <section className="hero" id="anasayfa">
        ...
        <div className="hero-route" aria-hidden="true">ÇEKMEKÖY · TAŞDELEN · SARIGAZİ · DUDULLU · SULTANBEYLİ</div>
        <div className="hero-live-card"><span className="live-dot" /><div><small>7/24 YOL YARDIM</small><strong>ÇEKMEKÖY MERKEZLİ</strong></div></div>
        <div className="hero-content">
          <div className="hero-kicker"><span /> ÇEKMEKÖY MERKEZLİ · 7/24 YOL YARDIM</div>
          <h1>ÇEKMEKÖY 7/24<br />OTO ÇEKİCİ<br /><em>&amp; KURTARMA</em></h1>
          <p className="hero-lead">Aracınız çalışmıyor veya yolda mı kaldınız? Çekmeköy merkezimizden Sancaktepe, Ümraniye ve Sultanbeyli'ye uygun yol yardım aracını yönlendirelim.</p>
          ...
          <div className="trust-row"><span><Clock3 /> 7 gün 24 saat</span><span><ShieldCheck /> Duruma uygun ekipman</span><span><MapPin /> Çekmeköy ve 3 çevre ilçe</span></div>
        </div>
      ...
      <section className="areas section-pad" id="bolgeler">
        <div className="section-heading"><div><small>HİZMET BÖLGELERİ</small><h2>ÇEKMEKÖY'DEN<br /><em>ÇEVRE İLÇELERE.</em></h2></div><p>Soğukpınar merkezli ekibimiz Çekmeköy, Sancaktepe, Ümraniye ve Sultanbeyli'de yol yardım ve araç taşıma desteği verir.</p></div>
        <div className="priority-areas">{["cekmekoy-oto-cekici", "sancaktepe-oto-cekici", "umraniye-oto-cekici", "sultanbeyli-oto-cekici"].map((slug, index) => { const area = locationPages.find((item) => item.slug === slug)!; return <Link href={`/${slug}`} key={slug}><span>0{index + 1}</span>{area.name}<ArrowUpRight /></Link>; })}</div>
        <div className="district-columns"><article><div className="neighborhoods-head"><h3>Çekmeköy mahalleleri</h3><span>{cekmekoyLinks.length} öncelikli nokta</span></div><div>{cekmekoyLinks.map((area) => <Link href={`/${area.slug}`} key={area.slug}>{area.name}</Link>)}</div></article>{priorityDistricts.map((district) => { const area = locationPages.find((item) => item.district === district); return area ? <article key={district}><div className="neighborhoods-head"><h3>{district} çevresi</h3><span>Öne çıkan noktalar</span></div><div>{area.localGuide[0]?.text}</div><p>{area.landmark}</p></article> : null; })}</div>
