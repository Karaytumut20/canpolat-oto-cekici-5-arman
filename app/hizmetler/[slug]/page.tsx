...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="area-page">
    <header className="site-header"><Link href="/" className="brand" aria-label="Arman Oto Kurtarma ana sayfa"><BrandLogo variant="horizontal" /></Link><nav><Link href="/hizmetler">Hizmetler</Link><Link href="/hizmet-bolgeleri">Bölgeler</Link><Link href="/cekmekoy-oto-cekici">Çekmeköy</Link><Link href="/uskudar-oto-cekici">Üsküdar</Link><Link href="/blog">Bilgi Merkezi</Link></nav><a className="header-call" href={`tel:${siteConfig.phone}`}><Phone size={18} /> {siteConfig.phoneDisplay}</a><MobileMenu links={[{ href: "/hizmetler", label: "Hizmetler" }, { href: "/hizmet-bolgeleri", label: "Hizmet Bölgeleri" }, { href: "/cekmekoy-oto-cekici", label: "Çekmeköy" }, { href: "/uskudar-oto-cekici", label: "Üsküdar" }, { href: "/blog", label: "Bilgi Merkezi" }]} /></header>
